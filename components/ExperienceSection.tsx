...
export function ExperienceSection() {
  return (
    <section id="experience" className="relative z-10 max-w-[1200px] mx-auto">
      <FadeIn>
      ...
      <div className="flex flex-col gap-8">
        {EXPERIENCES.map((exp, i) => (
          <FadeIn key={exp.role} delay={i * 80}>
            <div className="relative clip-corner-both bg-surface border border-[rgba(0,255,224,0.12)] lg:p-10 px-5 py-7 overflow-hidden">
              {/* Left accent bar */}
              <div className="absolute top-0 left-0 w-[3px] h-full bg-gradient-to-b from-accent to-accent2" />

              <div className="flex justify-between items-start flex-wrap gap-3 mb-4">
                <div>
                  <div className="text-2xl font-bold tracking-tight text-text1">
                    {exp.role}
                  </div>
                  <div className="text-[15px] text-accent2 font-semibold mt-1">
                    {exp.org}
                  </div>
                  <div className="font-mono text-[13px] text-text3 mt-0.5">
                    {exp.sub}
                  </div>
                </div>
              ...
              <p className="text-[15px] text-text2 leading-[1.8] mb-6">
                {exp.description}
              </p>
